...
- **2026-07-17 — Arabic legal name.** The registered English name is rendered
  as one isolated LTR value where legal identification is required. The Arabic
  transliteration in `src/config/site.ts` remains provisional until the trade
  license supplies an approved Arabic registered name.
...
- **2026-07-17 — Legal-page date.** `lastUpdated` is 2026-07-17 in both
  languages.
...
## QA evidence — current local remediation tree (2026-07-18)

### Automated verification

The current uncommitted GATE 2 remediation is based on local `main` at
`58fab48459b1`. The final receiving session must rerun these commands after any
additional edit:
...
tests. This statement describes implementation work only: `REVIEW_GATE2.md`
remains a FAIL for the baseline it reviewed until a fresh formal independent
GATE 2 re-review issues a new verdict.
